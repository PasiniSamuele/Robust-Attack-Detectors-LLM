...
#find all folders named run_0 recursively inside experiments_root
    ...
    for root, dirs, files in os.walk(opt.datasets_root):
        if opt.leaf_folder_name in dirs:
            results_json = os.path.join(root, opt.leaf_folder_name, opt.result_file_name)
    ...
    for run in runs: 
        for dataset in datasets:
            ...
            opt = Namespace(run=run, 
                            ...
                            test_results_file_name = opt.test_result_file_name
                            )
...
def add_parse_arguments(parser):
    ...
    parser.add_argument('--datasets_root', type=str, default='data/synthetic_datasets/task_detect_xss_simple_prompt/template_create_synthetic_dataset/prompt_parameters_medium_dataset/', help='Folder containing the synthetic datasets')
    ...
    parser.add_argument('--test_result_file_name', type=str, default='test_results.json', help='name of the results file for test set')
