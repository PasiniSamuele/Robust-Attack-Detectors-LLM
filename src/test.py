...
def test(opt):
    ...
    evaluation_namespace.top_k = None
    ...
    for k in opt.top_k:
        ...
        results_files = list(map(lambda x: f"{exp_folder}/{run}/{x}/{opt.synth_results_file_name}", top_k_experiments[k]["experiments"]))
...
def add_parse_arguments(parser):
    ...
    parser.add_argument('--synth_dataset_generation_mode', type=str, default="rag_few_shot", help='Synthetic dataset generation Mode')
