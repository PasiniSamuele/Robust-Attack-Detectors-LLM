...
def select_best_experiment(opt):
    ...
    df = df.drop(df[(df["model_name"] == "anthropic-claude-3-sonnet") & (df["temperature"] == 0.5)].index)
    ...
    df = df.drop(df[(df["model_name"] == "mixtral-8x7b-instruct-v01") & (df["temperature"] == 0.5)].index)
    ...
    #select the df rows where df["run"] contains 'generation_mode_zero_shot'
    ...
    validation_results  = {
        ...
        #"f2_baseline": float(((1 + 4) * df_baseline_best["precision"] *  df_baseline_best["recall"]) / (4 *  df_baseline_best["precision"] +  df_baseline_best["recall"])),
    }
...
def add_parse_arguments(parser):
    ...
    parser.add_argument('--output_file', type=str, default='experiments/task_detect_sqli_extended/template_create_function_readable/prompt_parameters_empty/best_experiment_sqli_f2.json', help='name of the file containing the summary of the experiments')
