...
plot_folder = "rq2_plots_sqli"
file = "plots_rq2_sqli.csv"
file_f2 = "plots_rq2_sqli_f2.csv"
...
colors_acc = sns.color_palette("summer_r", n_colors = len(top_ks)+3).as_hex()
colors_f2 = sns.color_palette("BuGn", n_colors = len(top_ks)+3).as_hex()

for i, top_k in enumerate(top_ks):
    output_plots_folder = os.path.join(plot_folder, f"top_{top_k}")
    plt.figure(figsize=(10, 10))
    os.makedirs(output_plots_folder, exist_ok=True)
    ax = sns.violinplot(data=plots_df, y=f"top_{top_k}_acc_diff", color = "deepskyblue",  alpha = 0.6, linewidth = 2, fill = True)
    ...
    ax = sns.violinplot(data=plots_df, y=f"top_{top_k}_acc_improvement",  color = colors_acc[i+2],linewidth = 2, fill = True,  alpha = 0.9)
    # ax.figure.set_size_inches(7,8)
    ax.set_title(f"Top {top_k}", fontsize=28) 
    ...
    plt.figure(figsize=(8, 9))
    ax = sns.violinplot(data=plots_df_f2, y=f"top_{top_k}_f2_improvement",  color = colors_f2[i+2],linewidth = 2, fill = True,  alpha = 0.9)
    # ax.figure.set_size_inches(7,8)
    ax.set_title(f"Top {top_k}", fontsize=28) 
